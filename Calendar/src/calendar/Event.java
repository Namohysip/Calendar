...
public class Event {
	...
	/**
	 * Default constructor that sets all dateTimes to their default state if
	 * such defaults exist
	 */
	public Event() {
		dateTimeS = null;
	...
	/**
	 * Resets the uid to a new unique id
	 */
	...
	/**
	 * Constructor that creates an event based on the paramaters passed in, as
	 * follows:
	 * 
	 * @param description
	 *            a brief description of the event
	 * @param summary
	 *            Essentially a title of the event
	 * @param location
	 ...
	 * @param classT
	 *            The type of class (Private, public)
	 * @param priority
	 *            the priority of the event (from 0 to 9, 1 = highest 9 = lowest
	 *            0 = undefined)
	 */
	public Event(String description, String summary, String location,
			String timeStart, String timeEnd, String classT, int priority) {
		dateTimeS = timeStart;
		dateTimeE = timeEnd;
		DateFormat df = new SimpleDateFormat("yyyyMMdd'T'HHmmss");
		Date date = new Date();
	...
	/**
	 * Gets the sequence number of the event (How many times it has been
	 * modified)
	 * 
	 * @return
	 */
	...
	/**
	 * Writes an ics file specifically for this event
	 * within the calendar
	 * @param filename
	 * @return
	 */
	public boolean writeics(String filename) {
		String calendar = "BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Rambutan\n";
		calendar += "CALSCALE:GREGORIAN\nMETHOD:PUBLISH\n";
		if (filename.endsWith(".ics")) {

			calendar += "BEGIN:VEVENT\nDTSTART:";
			calendar += getDateTimeS();
			calendar += "\nDTEND:" + getDateTimeE();
			...
			calendar += "\nSEQUENCE:" + Integer.toString(getSequence());
			calendar += "\nSTATUS:" + getStatus();
			calendar += "\nSUMMARY:" + getSummary();
